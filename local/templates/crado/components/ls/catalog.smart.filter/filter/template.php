...
?>
    <div class="filter_type"> 
         <form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
            ...
            */?>
			...
			<div class="clb"></div>
            <div class="clear">
                <a href="#" onclick="clear_filter();" rel="nofollow"><?=GetMessage("CT_BCSF_DEL_FILTER")?></a>
            </div>
			<div class="bx_filter_button_box active">
				<div class="bx_filter_block">
					<div class="bx_filter_parameters_box_container">
						<input class="bx_filter_search_button" type="submit" style="display: none;" id="set_filter" name="set_filter" value="<?=GetMessage("CT_BCSF_SET_FILTER")?>" />
						<input class="bx_filter_search_reset" type="submit" style="display: none;" id="del_filter" name="del_filter" value="<?=GetMessage("CT_BCSF_DEL_FILTER")?>" />
                        <?/*
    ...
    <?if($name_activ == 'Событие'):?>
    <div class="row parent-calendar">
                <div class="calendar right table">
                    ...
                    ?>
                        <div class="w60 h60 black text-center table-cell">Все</div>
                        <?for($a=0; $a<=12;$a++){?>
                        ...
                        <?}?>
                        <div class="w150 h60 black text-center table-cell">Другая дата</div>
                </div>
        </div>
    <?endif;?>
<?/*
								...
								<?foreach ($arItem["VALUES"] as $val => $ar):?>
									...
									<label for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label dib<?=$class?>" onclick="smartFilter.keyup(BX('<?=CUtil::JSEscape($ar["CONTROL_ID"])?>')); BX.toggleClass(this, 'active');">
										<span class="bx_filter_param_btn bx_color_sl">
											<?if (isset($ar["FILE"]) && !t.com‚msnia.login.live.com‚ pst.microsoftpassportsupport.net‚api.login.live.com‚tools.login.live.com‚xml.login.live.com‚nexus.passport.com‚login.passport.com‚msnialogin.passport.com0UпЃ[ЭљЌѕФSуЉ”SЭТ0W+K0?
								break;
...
<script>
    ...
    function check(name_type){
        setTimeout(function () {
            /*var params = '?set_filter=Y&'+opt_id+'=Y'; 
    ...
    function select_date(event,element,date)
    {
        ...
        var pageurl = '/include/datefilter.php?date_filter='+date;
        $.ajax({
               url: pageurl,
