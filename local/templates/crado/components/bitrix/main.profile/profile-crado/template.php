...
<div class="bwhite shadow edit-profile form">
    ...
    */?>
    <div class="row_form">
		<label>Имя</label>
		<input type="text" value="<?=$arResult["arUser"]['NAME']?>" name="NAME" class="required">
		<span class="inp_error empty_field table" style="display: none;"><div class="table-cell">Поле является обязательным для заполнения</div></span>
	</div>
    
    <div class="row_form">
		<label>Фамилия</label>
		<input type="text" value="<?=$arResult["arUser"]['LAST_NAME']?>" name="LAST_NAME">
		<span class="inp_error empty_field table" style="display: none;"><div class="table-cell">Поле является обязательным для заполнения</div></span>
	</div>
    ...
    <div class="row_form">
        <label>Аватар</label>
        <div class="file">
            <div class="text">Файл не выбран</div>
            <div class="button">Выберите файл</div>
        </div>
        <div class="inputfile">
            <input name="work-avatar" id="avatar" class="typefile" size="20" type="file"  accept='image/*' onchange="handleFileSelect(event,this)"/>
            
            <?//=$arResult["arUser"]["WORK_LOGO_INPUT"]?>
        </div>
        <input type="text" name="UF_IMG" id="new_avatar_in" style="display: none;"/>
        <span class="small-text">Максимальный размер файла 4 МБ</span>
        <div class="newfile-avatar" id="newfile-avatar">
        </div>
    </div>
 
    
    <div class="row_form row">
		<div class="col-6">
        ...
        <div class="col-5">
            ...
            <select name="PERSONAL_GENDER">
                <option value="M" <? if($arResult["arUser"]["PERSONAL_GENDER"] == 'M'){?>
                    selected
                    <?}?>
                >Мужской</option>
                <option value="F"  <? if($arResult["arUser"]["PERSONAL_GENDER"] == 'F'){?>
                    selected
                    <?}?>
                    >Женский</option>
            </select>
    ...
    <div class="row_form">
     ...
     <label>Электронная почта</label>
		<input type="text" autocomplete="off" value="<?=$arResult["arUser"]["EMAIL"]?>" name="EMAIL" onchange="setLogin(this)" class="email_required only_latin">
		<input type="hidden" value="<?=$arResult["arUser"]["EMAIL"]?>" name="LOGIN" class="login_input">

                <span class="inp_error email_field table" style="display: none;"><div class="table-cell">Неверный формат e-mail</div></span>
                <span class="inp_error latin_field table" style="display: none;"><div class="table-cell">Только латинские символы</div></span>
                <span class="inp_error empty_field table" style="display: none;"><div class="table-cell">Поле является обязательным для заполнения</div></span>
    ...
    <div class="row_form">
		...
		<span class="inp_error length_field table" style="display: none;"><div class="table-cell">Пароль должен быть не менне 6 символов</div></span>
                <span class="inp_error latin_field table" style="display: none;"><div class="table-cell">Пароль должен содержать только латинские буквы и цифры</div></span>
                <span class="inp_error empty_field table" style="display: none;"><div class="table-cell">Поле является обязательным для заполнения</div></span>
	</div>
    ...
    <div class="row_form">
            ...
            <span class="small-text"><?echo $arResult["GROUP_POLICY"]["PASSWORD_REQUIREMENTS"];?></span>
    		<span class="inp_error empty_field table" style="display: none;"><div class="table-cell">Поле является обязательным для заполнения</div></span>
            <span class="inp_error pass_field table" style="display: none;"><div class="table-cell">Пароль не совпадает</div></span>
        </div>
    <?/*
    ...
    <table class="data-table profile-table">
        ...
        */?>
    	<?// ******************** /User properties ***************************************************?>
    	<div class="text-center"><input type="submit" class="button" name="save" value="<?=GetMessage("SAVE")?>">
        <?/*
