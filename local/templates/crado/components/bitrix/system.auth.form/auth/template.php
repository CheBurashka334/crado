...
<form name="system_auth_form<?=$arResult["RND"]?>" method="post" target="_top" action="<?=$arResult["AUTH_URL"]?>">
    ...
    <div class="row table">
        ...
        <div class="auth-left col-6 table-cell">
        <?if($arResult["BACKURL"] <> ''):?>
                <input type="hidden" name="backurl" value="<?=$arResult["BACKURL"]?>" />
        <?endif?>
        <?foreach ($arResult["POST"] as $key => $value):?>
        ...
        <div class="registration-right col-6 table-cell">
            <div class="b">Регистрация. Быстро, удобно, легко!</div>
            <div>
                - Оставляйте комментарии к событиям и местам<br/>
                - Получайте персонализированные предложения<br/>
                - Добавляйте в избранное
            </div>
            <div class="text-center more">
                <noindex><a class="button" href="<?=$arResult["AUTH_REGISTER_URL"]?>" rel="nofollow"><?=GetMessage("AUTH_REGISTER")?></a></noindex>
            </div>
        </div>
    </div>
    <div class="social-auth">
        <div class="bx-auth-lbl"><?=GetMessage("socserv_as_user_form")?></div>
        <?
            /*$APPLICATION->IncludeComponent("bitrix:socserv.auth.form", "icons-crado", 
                    array(
                            "AUTH_SERVICES"=>$arResult["AUTH_SERVICES"],
                            "SUFFIX"=>"form",
                    ), 
        ...
        $APPLICATION->IncludeComponent("bitrix:socserv.auth.form", "icons-crado-default", 
					array(
						"AUTH_SERVICES"=>$arResult["AUTH_SERVICES"],
						"AUTH_URL"=>$arResult["AUTH_URL"],
						"POST"=>$arResult["POST"],
