...
<div data-skey="<?= md5($arParams['FAV_TYPE'] . $arParams['ELEMENT_ID'] . Coption::GetOptionString('asd.favorite', 'js_key')) ?>" class="col-3 button-favorite asd_<?= $arParams['BUTTON_TYPE'] ?>_button<? if ($arResult['FAVED'] == 'Y') echo ' asd_' . $arParams['BUTTON_TYPE'] . 'ed' ?>" id="asd_fav_<?= $arParams['ELEMENT_ID'] ?>">                              
    <div class="text">
        Нравится
    </div>
    <div class="heard">
         <svg class="icons"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#heard-icon"></use></svg>
    </div>
    <div class="znach" id="asd_count_<?= $arParams['ELEMENT_ID'] ?>">
        <?= $arResult['COUNT'] ?>
    </div>
</div> 
